...
- **Power-ups**: `POWER_TYPES` (`BOMB_TYPE`/`LASER_TYPE`/`DYE_TYPE`/`GRAVITY_TYPE`) are 1-block pieces. `clearLines()` sets `pendingPowerUp` once `lines` reaches a multiple of `POWERUP_LINE_INTERVAL`, forcing the next `randomPiece()` call to return one of them. `applyPowerEffect()` clears the piece's own cell then dispatches to `applyBomb()` (3×3 clear), `applyLaser()` (clear row+column), or `applyDye()` (clear the most frequent board color) — all mutate `board` directly and run before `clearLines()` so any newly completed rows still clear normally. Because those three remove blocks from the middle of the stack, `applyPowerEffect()` always finishes with `applyGravityCompact()` (drop each column's blocks to remove internal gaps), whether or not the locked piece was `GRAVITY_TYPE` itself — otherwise the holes they leave behind are unreachable by falling pieces and permanently block `clearLines()` on those rows.
